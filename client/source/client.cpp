...
void load_resources(HWND hwnd) {
  AddFontResourceEx(L"assets/IPix.ttf", FR_PRIVATE, 0);

  atlas_1p_idle_up.load(L"assets/img/hajimi_idle_back_%d.png", 4);
  atlas_1p_idle_down.load(L"assets/img/hajimi_idle_front_%d.png", 4);
  atlas_1p_idle_left.load(L"assets/img/hajimi_idle_left_%d.png", 4);
  atlas_1p_idle_right.load(L"assets/img/hajimi_idle_right_%d.png", 4);
  atlas_1p_run_up.load(L"assets/img/hajimi_run_back_%d.png", 4);
  atlas_1p_run_down.load(L"assets/img/hajimi_run_front_%d.png", 4);
  atlas_1p_run_left.load(L"assets/img/hajimi_run_left_%d.png", 4);
  atlas_1p_run_right.load(L"assets/img/hajimi_run_right_%d.png", 4);
  atlas_2p_idle_up.load(L"assets/img/manbo_idle_back_%d.png", 4);
  atlas_2p_idle_down.load(L"assets/img/manbo_idle_front_%d.png", 4);
  atlas_2p_idle_left.load(L"assets/img/manbo_idle_left_%d.png", 4);
  atlas_2p_idle_right.load(L"assets/img/manbo_idle_right_%d.png", 4);
  atlas_2p_run_up.load(L"assets/img/manbo_run_back_%d.png", 4);
  atlas_2p_run_down.load(L"assets/img/manbo_run_front_%d.png", 4);
  atlas_2p_run_left.load(L"assets/img/manbo_run_left_%d.png", 4);
  atlas_2p_run_right.load(L"assets/img/manbo_run_right_%d.png", 4);

  loadimage(&img_ui_1, L"assets/img/ui_1.png");
  loadimage(&img_ui_2, L"assets/img/ui_2.png");
  loadimage(&img_ui_3, L"assets/img/ui_3.png");
  loadimage(&img_ui_fight, L"assets/img/ui_fight.png");
  loadimage(&img_ui_textbox, L"assets/img/ui_textbox.png");
  loadimage(&img_background, L"assets/img/background.png");

  load_audio(L"assets/audio/bgm.mp3", L"bgm");
  load_audio(L"assets/audio/1p_win.mp3", L"1p_win");
  load_audio(L"assets/audio/2p_win.mp3", L"2p_win");
  load_audio(L"assets/audio/click_1.mp3", L"click_1");
  load_audio(L"assets/audio/click_2.mp3", L"click_2");
  load_audio(L"assets/audio/click_3.mp3", L"click_3");
  load_audio(L"assets/audio/click_4.mp3", L"click_4");
  load_audio(L"assets/audio/ui_1.mp3", L"ui_1");
  load_audio(L"assets/audio/ui_2.mp3", L"ui_2");
  load_audio(L"assets/audio/ui_3.mp3", L"ui_3");
  load_audio(L"assets/audio/ui_fight.mp3", L"ui_fight");
...
void login_to_server(HWND hwnd) {
  ...
  if (!res || res->status != 200) {
    MessageBox(hwnd, L"Failed to login to server!", L"Error",
               MB_OK | MB_ICONERROR);
    exit(-1);
  }
  ...
  std::thread([&]() {
    while (running) {
      ...
      std::string body =
          std::to_string((id_player == 1 ? progress_1 : progress_2).load());
...
// int main()
{
  ...
  Player player1(atlas_1p_idle_up, atlas_1p_idle_down, atlas_1p_idle_left,
                 atlas_1p_idle_right, atlas_1p_run_up, atlas_1p_run_down,
                 atlas_1p_run_left, atlas_1p_run_right);
  Player player2(atlas_2p_idle_up, atlas_2p_idle_down, atlas_2p_idle_left,
                 atlas_2p_idle_right, atlas_2p_run_up, atlas_2p_run_down,
                 atlas_2p_run_left, atlas_2p_run_right);
  ...
  timer_cursor.set_time_out([&]() { cursro_blink = !cursro_blink; });
  ...
  while (true) {
    ...
    if (stage == Stage::Waiting) {
      ...
      if ((id_player == 1 && progress_1 >= num_total_char) ||
        ...
        stop_audio(L"bgm");
        MessageBox(hwnd, L"You Lost!", L"Game Over",
      ...
      player1.set_target(
          path.get_postion_at_progress((float)progress_1 / num_total_char));
      player2.set_target(
          path.get_postion_at_progress((float)progress_2 / num_total_char));

      player1.on_update(delta.count());
      player2.on_update(delta.count());

      camera_scene.look_at(
          (id_player == 1 ? player1.get_position() : player2.get_position()));
    }
    ...
    if (stage == Stage::Waiting) {
      ...
      switch (val_countdown) {
      case 3: {
        static const Rect rect_ui_3 = {1280 / 2 - img_ui_3.getwidth() / 2,
      ...
      case 2: {
        static const Rect rect_ui_2 = {1280 / 2 - img_ui_2.getwidth() / 2,
      ...
      case 1: {
        static const Rect rect_ui_1 = {1280 / 2 - img_ui_1.getwidth() / 2,
      ...
      case 0: {
        static const Rect rect_ui_fight = {
            ...
            720 / 2 - img_ui_fight.getheight() / 2, img_ui_fight.getwidth(),
      ...
      // draw ui
      if (stage == Stage::Racing) {
        static const Rect rect_ui_textbox = {
            0, 720 - img_ui_textbox.getheight(), img_ui_textbox.getwidth(),
            img_ui_textbox.getheight()};
        std::wstring wstr_line = converter.from_bytes(str_line_text[idx_line]);
        std::wstring wstr_completed =
            converter.from_bytes(str_line_text[idx_line].substr(0, idx_char));
        ...
        if (cursro_blink) {
          line(185 + textwidth(wstr_completed.c_str()), rect_ui_textbox.y + 65,
               185 + textwidth(wstr_completed.c_str()),
               rect_ui_textbox.y + 65 + 28);
        }
      }
    }
    FlushBatchDraw();
    ...
    milliseconds sleep_time =
        frame_duration -
        duration_cast<milliseconds>(steady_clock::now() - frame_start);
